...
// ─── Config ───────────────────────────────────────────────────────────────────
// Bookings are stored in localStorage with a shared key.
// For multi-device sharing, replace STORAGE_KEY logic with Firebase (see README).
const STORAGE_KEY = "padel_bookings_v1";
...
function generateSlots() {
  ...
  while (current + 90 <= end) {
    ...
    slots.push({
      id: current,
      label: `${fmt(current)} – ${fmt(current + 90)}`,
      startMin: current,
    });
    current += 90;
  }
...
// 24h rule: booking opens exactly 24h before slot start
...
const navBtn = {
  ...
  color: "#e2e8f0", borderRadius: 9, width: 36, height: 36,
  cursor: "pointer", fontSize: 18, fontWeight: 700,
  display: "flex", alignItems: "center", justifyContent: "center",
};
const labelStyle = {
  fontSize: 10.5, color: "#475569", fontWeight: 700,
...
export default function App() {
  ...
  // Modal state
  const [modal, setModal] = useState(null); // { slot, date }
  ...
  // Sync from storage when tab regains focus (simulates multi-device)
  useEffect(() => {
    const onFocus = () => setBookings(loadFromStorage());
  ...
  function getBooking(date, slotId) {
    return bookings[dateKey(date)]?.[slotId] || null;
  }
  ...
  function openBookModal(slot) {
    ...
    if (!check.ok) {
      ...
      showToast(
        `Booking opens ${e.toLocaleDateString("en-US", { weekday: "short", month: "short", day: "numeric" })} at ${fmt2(e.getHours())}:${fmt2(e.getMinutes())}.`,
        "error"
      );
      return;
    }
            ...
            {weekDays.map(day => {
              ...
              return (
                <button key={day.toISOString()} onClick={() => setSelectedDate(day)} style={{
                  ...
                  opacity: isPast ? 0.45 : 1, transition: "all 0.16s",
                }}>
          ...
          <div style={{
            ...
            <div style={{ display: "flex", gap: 10, marginBottom: 18 }}>
              <div style={{ flex: 1 }}>
                ...
                <select value={phase}
                  onChange={e => { setPhase(e.target.value); setErrors(er => ({ ...er, phase: false })); }}
                  style={{
                    ...inputBase,
                    borderColor: errors.phase ? "#ef4444" : "rgba(255,255,255,0.1)",
                    cursor: "pointer", appearance: "none",
                    color: phase ? "#e2e8f0" : "#475569",
                  }}>
              ...
              {[0, 1, 2, 3].map(i => (
                <div key={i} style={{ display: "flex", gap: 8, alignItems: "center" }}>
                  ...
                  <input placeholder="First name"
                    value={players[i].first}
                    onChange={e => {
                      const u = [...players]; u[i] = { ...u[i], first: e.target.value };
                      setPlayers(u); setErrors(er => ({ ...er, [`p${i}f`]: false, playersMsg: null }));
                    }}
                  ...
                  <input placeholder="Last name"
                    value={players[i].last}
                    onChange={e => {
                      const u = [...players]; u[i] = { ...u[i], last: e.target.value };
                      setPlayers(u); setErrors(er => ({ ...er, [`p${i}l`]: false, playersMsg: null }));
                    }}
      ...
      {cancelTarget && (
        <div style={{
          ...
          <div style={{
            ...
            <p style={{ margin: "0 0 22px", color: "#64748b", fontSize: 13, textAlign: "center" }}>
      ...
      {toast && (
        <div style={{
          ...
          background: toast.type === "error"
            ? "linear-gradient(135deg, #7f1d1d, #3b0a0a)"
